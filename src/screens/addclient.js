...
//write a clinet
import { writeClient } from '../functions/saveValue';
//read all clients
...
export default function AddClient({ navigation, route }) {
    ...
    const saveClient = () => {
        ...
        if (name != undefined) {
            ...
            client['image'] = image != undefined ? String(image.base64) : "";
            writeClient(client).then(res => {
                setLoading(true)
                alert('Client added');
                navigation.goBack();
            })
        ...
        return (
            <KeyboardAvoidingView behavior={Platform.OS === "ios" ? 'padding' : nul} style={styles.container} >
                <SafeAreaView style={{ flex: 1 }}>
                    <View style={{ marginHorizontal: onePerWidth * 10, width: onePerWidth * 90, alignSelf: 'center', marginTop: onePerHeight * 3, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                        <TouchableOpacity>
                    ...
                    <TouchableWithoutFeedback style={{ flex: 1 }} onPress={() => { Keyboard.dismiss() }}>
                        <View style={[styles.inner]}>
                            ...
                            <View style={{ width: onePerWidth * 80, alignSelf: 'center', marginTop: onePerHeight * 2 }}>
                                <TextInput style={[styles.input]} keyboardType='name-phone-pad' placeholderTextColor="#00000070" placeholder='My Client Name' onChangeText={setName} />
